...
function addMarkers(rows) {
  ...
  rows.forEach(row => {
    ...
    const placeId = row.globalid || ""; // internal key
...
document.getElementById("surveyForm").addEventListener("submit", async (evt) => {
  ...
  if (!APPS_SCRIPT_URL) {
    status.textContent = "Set APPS_SCRIPT_URL in app.js first.";
    return;
  }
  ...
  try {
    ...
    if (out.ok) {
      status.textContent = "Submitted! Thanks — your submission is recorded.";
      evt.target.reset();
    } else {
...
(async function init() {
  ...
  const baselineRows = await loadBaseline();
  addMarkers(baselineRows);
})();
